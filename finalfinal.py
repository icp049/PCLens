...
# Get month from start_date
    ...
    filtered_parts = []
    month_info = []  # store for labeling
    change_notes = []
    prev_count = None

    for m in month_range:
        pcs_now = monthly_site_pcs.get(site, {}).get(m, set())
        current_count = len(pcs_now)

        if prev_count is not None:
            if current_count > prev_count:
                change_notes.append(f"NOTE: {current_count - prev_count} PC(s) added starting {m.strftime('%B %Y')}.")
            elif current_count < prev_count:
                change_notes.append(f"NOTE: {prev_count - current_count} PC(s) removed starting {m.strftime('%B %Y')}.")
    ...
    if filtered_parts:
            ...
            filtered = pd.DataFrame(columns=['Timestamp', 'ActivePCs'])
...
# Slice ticks and labels
    ...
    ax.set_title(f"PC Usage at {site}", fontsize=14)
