...
def update_plot():
    ...
    if not site or site not in site_timelines:
        root.after(0, lambda:status_label.configure(text="⚠ Please select a branch that has data loaded."))
        return
    ...
    if not start_str or not end_str:
        root.after(0, lambda:status_label.configure(text="⚠ Please select both start and end dates."))
        return
    ...
    if not filtered_parts:
        root.after(0, lambda:status_label.configure(text="⚠ No data to show for selected range."))
...
def load_current_file():
    cfg = load_config()
    current_path = cfg.get("current_file") or os.path.join(get_data_dir(), "current.xlsx")

    if not os.path.exists(current_path):
        root.after(0, lambda:status_label.configure(text="📂 No saved file. Click Import File to begin."))
        return
...
def reset_ui():
    ...
    site_timelines.clear()
    monthly_site_pcs.clear()
    root.after(0, lambda:status_label.configure(text="📂 Please load an Excel file to begin."))
