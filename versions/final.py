...
# Get month from start_date
    pcs_this_range = set()
    month_range = pd.period_range(start_date.to_period('M'), (end_date - pd.Timedelta(days=1)).to_period('M'))

    for m in month_range:
        pcs_this_range.update(monthly_site_pcs.get(site, {}).get(m, set()))

    monthly_pc_count = len(pcs_this_range)

    # ✅ Handle case: no PCs in selected range
    if monthly_pc_count == 0:
        month_range_str = f"{month_range[0]} to {month_range[-1]}"
        status_label.configure(text=f"⚠ No PC records found for {site} from {month_range_str}")
        return

    # ✅ Compute required count
    required_count = int(np.ceil((pct_required / 100) * monthly_pc_count)) or 1
...
# Slice ticks and labels
    ...
    text=f"🖥 {len(active_pc_names)}/{monthly_pc_count} PCs contributed during qualified time blocks (≥{pct_required}%)\n" + time_block
)
...
def load_data(file_path, progress_widget, label_widget, cancelled):
    ...
    for site in df['Site'].unique():
        ...
        for _, row in site_df.iterrows():
            ...
            login_time = login
            month = login_time.to_period('M') if pd.notna(login_time) else None
            if month:
